...
class FencingEnemy(Enemy):
    ...
    def update(self) -> None:
        if self.__direction == "right":
            self.x += 2
            if self.x >= self.game.screen_width - self.size / 2:
                self.__direction = "down"
        elif self.__direction == "down":
            self.y += 2
            if self.y >= self.game.screen_height - self.size / 2:
...
class UniqueEnemy(Enemy):
    ...
    def update(self) -> None:
        # Implement unique update logic here
        pass
...
class EnemyGenerator:
    ...
    def __init__(self, game: "TurtleAdventureGame", level: int):
        ...
        self.__enemy_creation_intervals = [500, 300, 200, 100]
    ...
    def create_enemy(self) -> None:
        ...
        """
        enemy_types = [RandomEnemy, ChasingEnemy, FencingEnemy, UniqueEnemy]
        colors = ['red', 'purple', 'orange', 'blue', 'limegreen', 'magenta']
        sizes = [10, 20, 30, 40, 50]
        new_enemy = random.choice(enemy_types)(self.__game, random.choice(sizes), random.choice(colors))
        ...
        if self.__level < len(self.__enemy_creation_intervals):
            self.__game.after(self.__enemy_creation_intervals[self.__level - 1], self.create_enemy)
